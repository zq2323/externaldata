  ...
  server <- function(input, output) {
    ...
    observeEvent(input$download_position,{
      if(input$download_position != ""){
        ...
        path2<- paste0(input$download_position,'/',str_replace(filename()[3],'/',''),'/',str_replace(str_replace(str_replace(filename()[2],'/',''),':',''),' ',''))
        if(!dir.exists(path2)){
          dir.create(path2)
        }
        vals$Data <- path2
    ...
    observeEvent(input$download_data,{
      ...
      if(rmd$Data !=''){
        if(rmd$Data == toupper(inputmd5())){
          mdline<-''
          mdline[1]<-'---------------文件下载成功---------------'
          mdline[2]<-paste0('项目编号：',filename()[2])
          mdline[3]<-paste0('文件类型: ',filetype())
          mdline[4]<-paste0('文件名： ',filename2())
          mdline[5]<-paste0('下载ID：', filename()[3])
