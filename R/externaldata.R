externaldata<-function(){
  test <- file("test.log")
  sink(test,append = TRUE)
  sink(test,append = TRUE,type = "message")
  if(!require(pacman)){install.packages('pacman')}
  library(pacman)
  p_load('shiny','stringr','caTools','shinyjs','RCurl')
  
  ui<-shinyUI(fluidPage(
    
    useShinyjs(),
    tags$div(style = "height: 10em;"),
    column(5,align='center',offset = 3,
           h2('外部数据下载'),
           tags$div(style = "height: 2em;"),
           textInput('username','用户名 :'),
           passwordInput('psw','密码 :'),
           textInput('link','链接:'),
           textInput('md5input','输入MD5 :'),
           tags$div(style = "height: 1em;"),
           selectInput('download_position','选择下载位置:',choices=c('','C:','D:','E:','F:'),selected = NULL),
           tags$div(style = "height: 1em;"),
           textOutput('remd5'),
           tags$div(style = "height: 1em;"),
           actionButton('download_data','执行'),
           tags$div(style = "height: 1em;"),
           textOutput('dstatus')
    )
  )
  )
  server <- function(input, output) {
    
    elink <- reactive({
      enc2native(str_trim(input$link))
    })
    
    
    
    filename<- reactive({
      lin <- elink()
      fname<- str_replace(str_extract(lin,'filedata=.*'),'filedata=','')
      fdecode <- base64decode(fname,'character')
      fname<-str_split(fdecode,',')
      fname <- unlist(fname)
      fname
    })
    
    durl <- reactive({
      uname<-input$username
      password<-input$psw
      lin <- elink()
      ben64<-base64encode(paste0(uname,'!',password))
      url<-sub('info=.*?token',paste0('info=',ben64,'&token'),lin)
      url <- str_replace_all(url,'\\|','\\&')
      url <- paste0(url,'&pid=',filename()[1])
      enc2native(url)
    })
    
    filename2 <- reactive({
      lin <- elink()
      fname2 <- str_replace(str_replace(str_extract(lin,'filename=.*?\\.'),'filename=',''),'\\.','')
      enc2native(fname2)
    })
    
    filetype<-reactive({
      lin <-elink()
      type<-str_replace(str_extract(str_extract(lin,'filename=.*?info'),'\\..*?\\|'),'\\|','')
      type
    })
    
    vals<-reactiveValues()
    rmd <-reactiveValues()
    rmd$Data<-''
    vals$Data<-'E:'
    observeEvent(input$download_position,{
      if(input$download_position != ""){
        path0 <- paste0(input$download_position,'/','EDC_DATA')
        if(!dir.exists(path0)){
          dir.create(path0)
        }
        path1<- paste0(path0,'/',str_replace(filename()[3],'/',''))
        if(!dir.exists(path1)){
          dir.create(path1)
        }
        path2<- paste0(path1,'/',str_replace(str_replace(str_replace(filename()[2],'/',''),':',''),' ',''))
        if(!dir.exists(path2)){
          dir.create(path2)
        }
        vals$Data <- path2
      }
    }
    )
    
    inputposition <- reactive({
      input$diwnload_position
    })
    
    
    status<-reactiveValues(changevalue1=FALSE,changevalue2=FALSE)
    
    
    
    observeEvent(input$download_data,{
      f <- CFILE(paste0(vals$Data,'/',filename2(),filetype()),mode='wb')
      curlPerform(url = durl(),writedata=f@ref)
      RCurl::close(f)
      commands<-paste0('CertUtil -hashfile ',paste0(vals$Data,'/',filename2(),filetype()),' MD5 ','> ',paste0(vals$Data,'/',filename2(),'-','md5.txt'))
      shell(commands)
      con<-file(paste0(vals$Data,'/',filename2(),'-','md5.txt'))
      mdline<-readLines(con,n=-1)
      rmd$Data <- toupper(str_replace_all(mdline[2],' ',''))
      if(rmd$Data !=''){
        if(rmd$Data == toupper(inputmd5())){
          mdline<-''
          mdline[1]<-'---------------文件下载成功---------------'
          mdline[2]<-paste0('项目编号：',filename()[3])
          mdline[3]<-paste0('文件类型: ',filetype())
          mdline[4]<-paste0('文件名： ',filename2())
          mdline[5]<-paste0('下载ID：', filename()[2])
          write.table(mdline,con,row.names = F,col.names = F,quote = F)
        }else{
          mdline<-''
          mdline[1]<-'---------------文件下载失败，md5与服务器不一致，请重新下载，或与外部数据管理员联系---------------'
          mdline[2]<-paste0('本次下载文件MD5：',rmd$Data)
          mdline[3]<-paste0('服务器文件MD5：',toupper(inputmd5()))
          write.table(mdline,con,row.names = F,col.names = F,quote = F)
        }
      }
    }
    )
    
    inputmd5<-reactive({
      if(!is.null(input$md5input)){
        imd5 <- input$md5input
        imd5
      }
    })
    
    
    output$remd5 <- renderText(
      if(rmd$Data !=''){
        if(rmd$Data == toupper(inputmd5())){
          paste0('下载文件MD5 ：',rmd$Data,'       验证结果 : MD5一致')
        }else{
          paste0('下载文件MD5 ：',rmd$Data,'       验证结果 : MD5不一致')
        }
      }else{
        'MD5待验证'
      }
      
    )
    
    
    
    observe({
      if (rmd$Data !=''){
        output$dstatus <- renderText(paste0('下载完成，文件下载至 ',paste0(vals$Data,'/',filename2(),filetype()),'已关闭页面'))
        status$changevalue1 <- TRUE
      }else
      {
        output$dstatus <- renderText('等待下载')
      }
    })
    
    observe({
      if(status$changevalue1){
        
        status$changevalue2 <- TRUE
        
        Sys.sleep(5)
      }
      
    })
    
    observe({
      if(status$changevalue2){
        stopApp()
      }
    })
    
    
  }
  runApp(list(ui=ui,server=server),host="127.0.0.1",port=7264,launch.browser = TRUE)
  
  cat("\014")
  sink()
  sink(type="message")
  base::close(test)
  file.remove("test.log")
}